...
def gated_memory_t2i(input_0, input_1):
        input_cat = torch.cat([input_0, input_1], dim=2)
        input_1 = torch.tanh(linear_t2i(input_cat))
        gate = torch.sigmoid(gate_t2i(input_cat))
        output = input_0 * gate + input_1 * (1 - gate)
        return output

def gated_memory_i2t(input_0, input_1):
        input_cat = torch.cat([input_0, input_1], dim=2)
        input_1 = torch.tanh(linear_i2t(input_cat))
        gate = torch.sigmoid(gate_i2t(input_cat))
        output = input_0 * gate + input_1 * (1 - gate)
        return output
...
def xattn_score_Text_IMRAM(images, captions, cap_lens, lambda_softmax):
    """
    Images: (n_image, n_regions, d) matrix of images
    Captions: (n_caption, max_n_word, d) matrix of captions
    CapLens: (n_caption) array of caption lengths
    """
    similarities = [[] for _ in range(3)]
    n_image = images.size(0)
    n_caption = captions.size(0)
    images = images.float()
    captions_all = captions.float()
    for i in range(n_caption):
        # Get the i-th text description
        n_word = cap_lens[i]
        cap_i = captions[i, :n_word, :].unsqueeze(0).contiguous()
        ...
        context = images
        for j in range(3):
            weiContext, attn = func_attention(query, context, smooth=lambda_softmax)
            ...
            query = gated_memory_t2i(query, weiContext)
...
def xattn_score_t2i(images, captions, cap_lens, lambda_softmax):
    ...
    for i in range(n_caption):
        ...
        cap_i = captions[i, :n_word, :].unsqueeze(0).contiguous()
       # cap_i = captions[i, :n_word].unsqueeze(0).unsqueeze(0).contiguous()
        # --> (n_image, n_word, d)
        cap_i_expand = cap_i.repeat(n_image, 1, 1)
        """
            word(query): (n_image, n_word, d)
...
def xattn_score_i2i(images, lambda_softmax):
    """
    Images: (batch_size, n_regions, d) matrix of images
    """
    similarities = []
    ...
    for i in range(n_image):
        ...
        """
            query: (n_image, n_region, d)
            context: (n_image, n_region, d)
            weiContext: (n_image, n_region, d)
            attn: (n_image, n_region, n_region)
        """
        weiContext, attn = func_attention(images, img_i_expand, smooth=lambda_softmax)
        # (n_image, n_region)
        row_sim = F.cosine_similarity(images, weiContext, dim=2)
    ...
    # Ignore self-similarity by setting diagonal elements to 0
    similarities = similarities * (1 - torch.eye(n_image, device=similarities.device))
...
def image_similarity(images):
    """
    Compute pairwise cosine similarity between images in a batch.

    Args:
    images: (batch_size, n_regions, d) matrix of image features
            where batch_size is the number of images,
            n_regions is the number of regions/features per image,
            d is the feature dimension.

    Returns:
    similarities: (batch_size, batch_size) matrix of image-to-image similarities
    """
    # Compute image-level features by averaging region features (if needed)
    # Assuming each image is represented by multiple regions (e.g., features from a CNN)
    image_feats = images.mean(dim=1)  # (batch_size, d), averaging across regions

    # Normalize image features to unit vectors (L2 normalization)
    image_feats_norm = F.normalize(image_feats, p=2, dim=1)

    # Compute pairwise cosine similarity between images
    similarities = torch.mm(image_feats_norm, image_feats_norm.t())  # (batch_size, batch_size)
...
def rank_matrix(sim_matrix):
    ...
    sim_rank = diag_mask * sigmoid(tensor=sim_expand, temp=0.001)
...
def compute_pool_weights_fixed(lengths, features):
    # lengths 是一个固定值，表示所有样本的序列长度
    max_len = int(lengths)  # lengths 是一个标量，代表固定的最大长度
    mask = torch.ones(features.size(0), max_len, 1).to(features.device)  # 所有位置都有效
    return mask

def feaselect_fixed(features, lengths):
    # lengths 是一个标量，表示固定长度
    mask = compute_pool_weights_fixed(lengths, features)

    # 不需要裁剪，因为每个样本的长度是固定的
    sorted_features = features.masked_fill(mask == 0, -10000)
    sorted_features = sorted_features.sort(dim=1, descending=True)[0]  # 对特征进行排序，选择最大值
    sorted_features = sorted_features.masked_fill(mask == 0, 0)

    # 选择池化特征，最大值
    pooled_features = sorted_features[:, 0, :]  # 每个序列的最大特征（第一个排序后的特征）

    pool_weights = None  # 不使用池化权重

    return pooled_features, pool_weights

def compute_pool_weights(lengths, features):
...
def feaselect( features, lengths):

        mask = compute_pool_weights(lengths, features)
...
#编码器组合
def compute_SCAN(self, im, s, s_l,add_data,direction, lambda_softmax):
    ...
    if direction == 't2i':
        ...
        scores_t2i = xattn_score_t2i(im, s, s_l, lambda_softmax)
        scores_1_t2i = xattn_score_t2i(image_feats_1, text_feats_1, s_l, lambda_softmax)
        scores_2_t2i = xattn_score_t2i(image_feats_2, text_feats_2, s_l, lambda_softmax)

        tscores = scores_t2i + scores_1_t2i + scores_2_t2i

        scores_i2t = xattn_score_i2t(im, s, s_l, lambda_softmax)
        # scores_0 = xattn_score_i2t(image_feats_0, text_feats_0, s_l, lambda_softmax)
        scores_1_i2t = xattn_score_i2t(image_feats_1, text_feats_1, s_l, lambda_softmax)
        scores_2_i2t = xattn_score_i2t(image_feats_2, text_feats_2, s_l, lambda_softmax)
        iscores = scores_i2t + scores_1_i2t + scores_2_i2t

        scores = (0.5 * tscores) + (0.5 * iscores)

    else:
        raise ValueError("unknown first norm type")

    result_name = "/Workplace_sdb/hwb/project/HAT_init/runs_120/margin"
    # the result of the belt/alpha
    belt_alpha = 0.3
    margin = 0.2
    ...
    for i in range(scores_dig.size(0)):
        ...
        if torch.max(scores[i], 0)[0].item() == pos:
            ...
            #####
            negative_example.append(neg + self.hparams.config["margin"])

    pos_count = len(positive_example)
    if pos_count > 45:
        print("pos_count > 45")
        pos = torch.tensor(positive_example)
        neg = torch.tensor(negative_example)
        ...
        C = (mean_neg * stnd_pos).pow(2) - (mean_pos * stnd_neg).pow(2) + 2 * (stnd_pos * stnd_neg).pow(
            2) * torch.log(belt_alpha * stnd_neg / (stnd_pos) + 1e-8)

        thres = self.hparams.config["margin"]
        stnd_pos = stnd_pos.item()
        stnd_neg = stnd_neg.item()
        mean_pos = mean_pos.item()
        mean_neg = mean_neg.item()
        ...
        if E > 0:
            ...
            if up_margin < 0.0:
                print("1")
                up_margin = 0.0
                self.hparams.config["margin"] = thres
            elif up_margin > 1.0:
                print("2")
                up_margin = 0.2
                self.hparams.config["margin"] = 0.1 * up_margin + 0.9 * thres
            else:
                print("3")
                self.hparams.config["margin"] = 0.1 * up_margin + 0.9 * thres
            print("print margin")
            file_result = open(result_name + '/' + '%s_%s_%s_margin.txt' % (
                data_name, direction, belt_alpha), 'a')
            file_result.write(str(up_margin) + "\t" + str(self.hparams.config["margin"]) + "\n")
            file_result.close()
        ...
        cost_s = (self.hparams.config["margin"] + scores - d1).clamp(min=0)

        cost_im = (self.hparams.config["margin"] + scores - d2).clamp(min=0)
...
#
    ...
    #loss = compute_SCAN(pl_module, image_feats, text_feats, text_lengths,add_data,
                 #       pl_module.hparams.config["direction"], pl_module.hparams.config["lambda_softmax"])
...
#         sims_1 = shard_xattn_i2t(img_embs_1, cap_embs_1, stc_lens, pl_module.hparams.config["lambda_softmax"],
#                                  shard_size=128)
#         sims_2 = shard_xattn_i2t(img_embs_2, cap_embs_2, stc_lens, pl_module.hparams.config["lambda_softmax"],
#                                  shard_size=128)
...
#         sims_1 = shard_xattn_t2i(img_embs_1, cap_embs_1, stc_lens, pl_module.hparams.config["lambda_softmax"],
#                                  shard_size=128)
#         sims_2 = shard_xattn_t2i(img_embs_2, cap_embs_2, stc_lens, pl_module.hparams.config["lambda_softmax"],
#                                  shard_size=128)
#         sims_all = sims + sims_1 + sims_2
#     else:
#         sims_i2t = shard_xattn_i2t(img_embs, cap_embs, stc_lens, pl_module.hparams.config["lambda_softmax"], shard_size=128)
#         # sims_0 = shard_xattn_i2t(img_embs_0, cap_embs_0, stc_lens, pl_module.hparams.config["lambda_softmax"], shard_size=128)
#         sims_1_i2t = shard_xattn_i2t(img_embs_1, cap_embs_1, stc_lens, pl_module.hparams.config["lambda_softmax"],
#                                  shard_size=128)
#         sims_2_i2t = shard_xattn_i2t(img_embs_2, cap_embs_2, stc_lens, pl_module.hparams.config["lambda_softmax"],
#                                  shard_size=128)
#         isims = sims_i2t+sims_1_i2t+sims_2_i2t
#         sims_t2i = shard_xattn_t2i(img_embs, cap_embs, stc_lens, pl_module.hparams.config["lambda_softmax"], shard_size=128)
#         # sims_0 = shard_xattn_t2i(img_embs_0, cap_embs_0, stc_lens, pl_module.hparams.config["lambda_softmax"], shard_size=128)
#         sims_1_t2i = shard_xattn_t2i(img_embs_1, cap_embs_1, stc_lens, pl_module.hparams.config["lambda_softmax"],
#                                  shard_size=128)
#         sims_2_t2i = shard_xattn_t2i(img_embs_2, cap_embs_2, stc_lens, pl_module.hparams.config["lambda_softmax"],
#                                  shard_size=128)
#         tsims = sims_t2i+sims_1_t2i+sims_2_t2i
#
#         sims_all = (0.5*isims)+(0.5*tsims)
#
#
#     """
#
