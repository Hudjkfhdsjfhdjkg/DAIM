...
def i2t_SCAN(sims, npts=None, return_ranks=False):
    ...
    with open("i2t_only_vitbert.txt", "a") as f:
        for i in range(len(ranks)):
            item = ranks[i]
            f.write(f"image {i}-->{item}\n")
            top_10_indices = np.argsort(sims[i])[::-1][:15]  # Retrieve the top 15 closest text indices
...
def t2i_SCAN(sims, npts=None, return_ranks=False):
    ...
    with open("t2i_only_vitbert.txt", "a") as f:
        for i in range(len(ranks)):
            item = ranks[i]
            f.write(f"text {i}-->{item}\n")
            top_10_indices = np.argsort(sims[i])[::-1][:15]  # Retrieve the top 10 closest text indices
